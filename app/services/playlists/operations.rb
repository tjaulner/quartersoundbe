module Playlists
    module Operations
        def self.new_playlist(params, current_user)
            playlist = current_user.playlists.new(playlist_name: params[:playlist_name])
        ...
        def self.update_playlist(params)
            ...
            return ServiceContract.success(playlist) if playlist.update(playlist_name: params[:playlist_name])
